...
ace = tk.Tk()
ace.title('CRIME RECORD')
ace.iconbitmap('police-order.ico')

#GEOMETRY

width , height = 500, 500
screen_width, screen_height  = ace.winfo_screenwidth(), ace.winfo_screenheight()
x, y = screen_width//2-width//2, screen_height//2 - width//2
ace.geometry(f'{width}x{height}+{x}+{y}')
...
def edit():
	global my_tree, result_frame, edit_frame, cr
	back()
	
	edit_frame.grid(row = 2, column = 0,sticky = 'w')

	user_label = tk.Button(edit_frame, text = 'USER DATA', font =('Unispace', 10), fg = '#223377', command = user_data)
	user_label.grid(row = 0, column = 0)
	criminal_label = tk.Button(edit_frame, text = 'CRIMINAL DATA', font =('Unispace', 10), fg = '#223377', command = criminal_data)
	criminal_label.grid(row = 0, column = 2)
	

	result_frame = tk.Frame(ace)
...
def user_data():
	...
	Rank_l = tk.Label(ent_frame, text = 'Rank', font = ('STENCIL',10))
	Rank_l.grid(row = 3, column = 0, pady = 5)
	...
	Rank_e = ttk.Combobox(ent_frame)
	Rank_e['values'] = ranks
	Rank_e.grid(row = 3, column = 1, pady = 5)
	Rank_e.current()
	...
	edit_data_b = tk.Button(ent_frame, font = ('Candara', 10), bg = '#D0F2CF', text = 'EDIT', command = edit_data, width = 10).grid( row = 6, column = 0, pady = 5, columnspan = 2)
...
def criminal_data():
	global ace, IDce, Ofce, Sce, Cce, CNce, CEnt, cr, ct
	back()
	ct.destroy()
	ct = ttk.Treeview(cr)
...
file_menu.add_command(label = 'View Complaints', command = vc)
...
logo = tk.PhotoImage(file ='ace.png')
logo = logo.subsample(3)
photo_label = tk.Label(ace, image = logo)


profile_frame = tk.Frame(ace)
option_frame = tk.LabelFrame(ace, text = 'Options', font = ('Lucida bright',10), width = 400, height = 300)
record_frame = tk.Frame(ace, width = 500, height = 300)
edit_frame = tk.Frame(ace, width = 500, height = 300)
ent_frame = tk.Frame(ace)
